...
var loadConfigFile = function (type, extra_url) {
	var json = null;
...
//
//	Jasonwang826 customize
//
	var configu = {};
	var user_config = $.urlParam('user_config')
	if( user_config!=0 )	{
		configu = loadConfigFile('extra',user_config);
		console.log({'configu':configu.options});
		config = $.extend(true, {}, config, configu);
	}
...
$(function(){
	...
	console.log({'extras':config.extras.extra_js});
	...
	console.log({'fileRoot':config.options.fileRoot});
	...
	console.log({'relPath':config.options.relPath});
	if(config.options.relPath === false) {
	...
	//
	//	Jasonwang826 customization
	//
		var $fileRootList = $('#fileRootList');
		if( config.options.fileRootList && config.options.fileRootList.length>0 )	{
			$fileRootList.show();
			$.each( config.options.fileRootList,
				function( i, e ) {
					$fileRootList.append( $('<option></option>').val(e.url).html(e.name) );
				});
			$fileRootList.change(function(e)	{
				var vFileRoot = $(this).val();
				getFolderInfo(vFileRoot);
				createFileTree();
			});
		}	else	{
			$fileRootList.append( $('<option></option>').val(config.options.fileRoot).html('--') );
		}
	...
	console.log({'autoload':config.options.autoload });
	...
	console.log({'searchBox':config.options.searchBox });
	/** load searchbox */
	if(config.options.searchBox === true)  {
	...
	$('#home').click(function() {
		...
		getFolderInfo($fileRootList.val());
	});
	...
	console.log({'multiple':config.upload.multiple});
	// Multiple Uploads
	...
	console.log({'customScrollbar':config.customScrollbar});
