<?
	...
	class JLocalControl extends JTemplateControl {
		public function _list() {
			...
			$arrOrderBy = isset($_GET['_orderBy']) ? $_GET['_orderBy'] : array('`slide`.`ord` ASC,`slide`.`id` DESC');
			$pager = $this->db->queryFetchPage( $sql, $pageNo, $rowPerPage, $arrOrderBy );
		//	list data
			$smarty = $this->createSmarty();
			$smarty->assign( 'pager', $pager );
			$smarty->assign( 'maxOrd', JTSlide::getMaxOrd('slide') );
		...
		public function _insert() {
			$r= new JTSlide($_SESSION['login_user_id']);
